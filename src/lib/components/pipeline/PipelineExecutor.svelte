<script lang="ts">
  ...
  async function executePipeline() {
    ...
    try {
      ...
      // Get user home directory using Tauri API
      let userHome = '';
      try {
        userHome = await invoke('get_user_home');
      } catch (error) {
        logger.warn({ message: 'Failed to get user home directory:', data: error });
        userHome = '';
      }
      ...
      logger.error({ message: 'Failed to execute pipeline:', data: error });
  ...
  async function updateProgress(executionId: string) {
    try {
      ...
      logger.error({ message: 'Failed to update progress:', data: error });
    }
  }
  ...
  async function cancelExecution() {
    ...
    try {
      ...
      logger.error({ message: 'Failed to cancel execution:', data: error });
