<script lang="ts">
  ...
  // Browser check - simplified for now
  ...
  $effect(() => {
    if ($connectionState.isConnected) {
      logger.info('Sidebar: Connection state changed to connected');
      loadNamespacesData();
    }
  });
  ...
  async function loadNamespacesData() {
    ...
    if (isLoadingNamespaces) {
      logger.info('Sidebar: Namespaces already loading, skipping...');
      return;
    }
    
    try {
      isLoadingNamespaces = true;
      logger.info('Sidebar: Loading namespaces via appStore...');
      await appStore.loadNamespaces();
    } catch (error) {
      logger.error('Sidebar: Failed to load namespaces:', error);
  ...
  function handleNamespaceChange(data: {namespace: string}) {
    const namespace = data.namespace;
    logger.info('Sidebar: Received namespace change event', { namespace });
        ...
        <div class="bg-slate-700/50 rounded-lg p-3">
          {#if $namespaceState.available.length > 0}
            <NamespaceSelector 
              ...
              variant="sidebar"
              on:namespaceChange={(e) => handleNamespaceChange(e.detail.namespace)}
            />
          {:else}
