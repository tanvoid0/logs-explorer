<script lang="ts">
  ...
  const { 
    ...
    viewMode = 'detailed',
    namespace = '',
    onFilterByTraceId = undefined,
    onFilterByDeployment = undefined,
    onFilterBySeverity = undefined,
    onPinStartTime = undefined,
    onPinEndTime = undefined
  } = $props<{
    log: K8sLog;
    viewMode?: 'detailed' | 'compact' | 'raw' | 'lean';
    namespace?: string;
    onFilterByTraceId?: ((data: { traceId: string }) => void);
    onFilterByDeployment?: ((data: { deploymentName: string }) => void);
    onFilterBySeverity?: ((data: { severity: string }) => void);
    onPinStartTime?: ((data: { timestamp: string }) => void);
    onPinEndTime?: ((data: { timestamp: string }) => void);
  }>();
  ...
  function handleTraceIdClick(traceId: string) {
    if (onFilterByTraceId) onFilterByTraceId({ traceId });
  }
  
  function handleDeploymentClick(deploymentName: string) {
    if (onFilterByDeployment) onFilterByDeployment({ deploymentName });
  }
  
  function handleSeverityClick(severity: string) {
    if (onFilterBySeverity) onFilterBySeverity({ severity });
  }

  function handlePinStartTime() {
    if (onPinStartTime) onPinStartTime({ timestamp: log.timestamp });
  }

  function handlePinEndTime() {
    if (onPinEndTime) onPinEndTime({ timestamp: log.timestamp });
  }
