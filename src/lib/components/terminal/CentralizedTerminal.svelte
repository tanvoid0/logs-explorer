<script lang="ts">
  ...
  // Reactive statements
  $effect(() => {
    logger.info('🔥 CENTRALIZED TERMINAL: Process history updated', { processes: $processHistory.length });
    logger.info('🔥 CENTRALIZED TERMINAL: Active process', { activeProcess: $activeProcess });
    logger.info('🔥 CENTRALIZED TERMINAL: Running processes', { count: $runningProcesses.length });
  ...
  async function runCommand(command: string) {
    ...
    logger.info('🔥 CENTRALIZED TERMINAL: Running command', { command });
    logger.info('🔥 CENTRALIZED TERMINAL: Working directory', { workingDirectory: workingDirectory || projectPath });
    
    try {
      await terminalStore.startProcess(command, [], workingDirectory || projectPath);
      currentCommand = command;
      logger.info('🔥 CENTRALIZED TERMINAL: Command started successfully');
    } catch (error) {
      logger.error('🔥 CENTRALIZED TERMINAL: Failed to start process', error);
      toastStore.error('Failed to start command');
    }
  }

  async function cancelCurrentProcess() {
    logger.info('🔥 CENTRALIZED TERMINAL: Cancel requested');
    logger.info('🔥 CENTRALIZED TERMINAL: selectedProcessId', { selectedProcessId });
    logger.info('🔥 CENTRALIZED TERMINAL: activeProcess', { activeProcess: $activeProcess });
    logger.info('🔥 CENTRALIZED TERMINAL: runningProcesses', { runningProcesses: $runningProcesses });
    ...
    if (!processToCancel && $activeProcess) {
      ...
      logger.info('🔥 CENTRALIZED TERMINAL: Using active process ID', { processToCancel });
    }
    
    if (!processToCancel && $runningProcesses.length > 0) {
      processToCancel = $runningProcesses[0].id;
      logger.info('🔥 CENTRALIZED TERMINAL: Using first running process ID', { processToCancel });
    }
    
    if (processToCancel) {
      logger.info('🔥 CENTRALIZED TERMINAL: Cancelling process', { processToCancel });
      
      try {
        const result = await terminalStore.cancelProcess(processToCancel);
        logger.info('🔥 CENTRALIZED TERMINAL: Cancel result', { result });
        ...
        logger.error('🔥 CENTRALIZED TERMINAL: Failed to cancel process', error);
        toastStore.error('Failed to cancel command');
      }
    } else {
      logger.info('🔥 CENTRALIZED TERMINAL: No process found to cancel');
  ...
  async function debugProcesses() {
    logger.info('🔥 CENTRALIZED TERMINAL: === DEBUG INFO ===');
    logger.info('🔥 CENTRALIZED TERMINAL: selectedProcessId', { selectedProcessId });
    logger.info('🔥 CENTRALIZED TERMINAL: activeProcess', { activeProcess: $activeProcess });
    logger.info('🔥 CENTRALIZED TERMINAL: runningProcesses', { runningProcesses: $runningProcesses });
    logger.info('🔥 CENTRALIZED TERMINAL: processHistory', { processHistory: $processHistory });
    
    try {
      const runningProcesses = await invoke('get_running_processes');
      logger.info('🔥 CENTRALIZED TERMINAL: Backend running processes', { runningProcesses });
    } catch (error) {
      logger.error('🔥 CENTRALIZED TERMINAL: Failed to get backend processes', error);
    }
    
    logger.info('🔥 CENTRALIZED TERMINAL: === END DEBUG ===');
  }
  ...
  function createNewTab() {
    logger.info('🔥 CENTRALIZED TERMINAL: Creating new tab');
    ...
    logger.info('🔥 CENTRALIZED TERMINAL: New tab created', { processId });
  }
  ...
  // Lifecycle
  onMount(() => {
    logger.info('🔥 CENTRALIZED TERMINAL: Component mounted');
  ...
  onDestroy(() => {
    logger.info('🔥 CENTRALIZED TERMINAL: Component destroyed');
    ...
    <div class="debug-actions">
      ...
      <div class="action-buttons">
        ...
        <button 
          class="action-btn debug-btn"
          onclick={() => {
            logger.info('🔥 DEBUG: Current process history:', { data: $processHistory });
            logger.info('🔥 DEBUG: Current active process:', { data: $activeProcess });
            logger.info('🔥 DEBUG: Current running processes:', { data: $runningProcesses });
          }}
