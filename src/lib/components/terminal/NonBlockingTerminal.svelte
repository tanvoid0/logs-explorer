<script lang="ts">
  ...
  async function runCommand(command: string) {
    logger.info({ message: '🔥 FRONTEND: runCommand called with:', command });
    if (isRunning) {
      logger.info('🔥 FRONTEND: Command already running, returning');
      return;
    }
    
    try {
      logger.info('🔥 FRONTEND: Starting command execution...');
      isRunning = true;
      ...
      logger.info({ message: '🔥 FRONTEND: Running command:', command });
      logger.info({ message: '🔥 FRONTEND: Parsed command:', cmd });
      logger.info({ message: '🔥 FRONTEND: Parsed args:', args });
      logger.info({ message: '🔥 FRONTEND: Working directory:', projectPath });
      
      // Use the new execute_command_live function
      logger.info('🔥 FRONTEND: Invoking execute_command_live...');
      ...
      logger.info('🔥 FRONTEND: Command completed:', result);
      ...
      logger.error('🔥 FRONTEND: Command execution error:', error);
      output += `\n❌ Error: ${error}\n`;
      terminalStatus = 'error';
    } finally {
      isRunning = false;
  ...
  async function cancelCommand() {
    ...
    try {
      logger.info('🔥 FRONTEND: Cancelling command...');
      ...
      logger.error('🔥 FRONTEND: Error cancelling command:', error);
  ...
  async function loadPackageJsonScripts() {
    ...
    try {
      ...
      logger.error('Failed to load package.json scripts:', error);
      packageJsonScripts = [];
    }
  }
  ...
  async function loadShellInfo() {
    try {
      ...
      logger.error('Failed to load shell info:', error);
    }
  }

  onMount(() => {
    // Set up event listener for real-time command output
    logger.info('🔥 FRONTEND: Setting up command output listener...');
    
    // Use a more robust event listener setup with retry mechanism
    const setupEventListener = async (retryCount = 0) => {
      try {
        logger.info(`🔥 FRONTEND: Setting up event listener (attempt ${retryCount + 1})...`);
        
        const unsub = await listen('command_output', (event) => {
          const line = event.payload as string;
          logger.info({ message: '🔥 FRONTEND: Received command output:', data: line });
          
          // Convert ANSI codes to HTML and append to output
          const htmlLine = ansiConverter.toHtml(line);
          logger.info({ message: '🔥 FRONTEND: Adding to output:', data: htmlLine });
          output += htmlLine + '\n';
          logger.info({ message: '🔥 FRONTEND: Output length after update:', data: output.length });
        });
        
        logger.info('🔥 FRONTEND: Command output listener set up successfully');
        unsubscribe = unsub;
      } catch (error) {
        logger.error('🔥 FRONTEND: Failed to set up command output listener:', error);
        
        // Retry up to 3 times with exponential backoff
        if (retryCount < 3) {
          const delay = Math.pow(2, retryCount) * 1000; // 1s, 2s, 4s
          logger.info(`🔥 FRONTEND: Retrying in ${delay}ms...`);
          setTimeout(() => setupEventListener(retryCount + 1), delay);
        } else {
          logger.error('🔥 FRONTEND: Failed to set up event listener after 3 attempts');
        }
      }
    };
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
          ...
          <Button 
            variant="outline" 
            onclick={() => {
              logger.info('🔥 FRONTEND: Testing output update...');
              output += '🧪 Test output: ' + new Date().toLocaleTimeString() + '\n';
              logger.info({ message: '🔥 FRONTEND: Output length after test:', data: output.length });
            }} 
            size="sm"
